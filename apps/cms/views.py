...
from .forms import WriteNewsForm,EditNewsCategoryForm,EditNewsForm
import os
import qiniu
...
def news_category(request):
    categories = NewsCategory.objects.all() # 把所有的分类取出来
    context = {
        'categories': categories
    }
...
def upload_file(request):
    ...
    with open(os.path.join(settings.MEDIA_ROOT,name),'wb') as fp:
        for chunk in file.chunks():
            fp.write(chunk)
            #settings.MEDIA_URL 这是存放上传文件的url
    url = request.build_absolute_uri(settings.MEDIA_URL+name)
    # # http://127.0.1:9000/media/111.jpg 返回的是这个效果
    return restful.result(data={'url':name})
...
def qntoken(request):
    ...
    bucket = settings.QINIU_BUCKET_NAME  #就是存储空间名字
